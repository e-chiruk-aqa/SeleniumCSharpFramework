    ...
    emit_drag(source, 'dragstart', delay, function () {
        ...
        emit_drag(target, 'dragenter', 1, function () {
            emit_drag(target, 'dragover', delay, function () {
                emit_drag(doc.elementFromPoint(x, y), 'drop', delay, function () {
                    emit_drag(source, 'dragend', delay, function () {
                        emit_mouse(doc.elementFromPoint(x, y), ['mouseup', 'pointerup'], delay);
                    });
                });
            });
        });
